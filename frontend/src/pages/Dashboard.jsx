...
import {
  BookOpen, Upload, Trash2, MessageSquare, Loader2,
  FileText, CheckCircle, XCircle, Clock, LogOut, RefreshCw,
} from "lucide-react";
...
export default function Dashboard() {
    ...
    try {
      ...
      res.data.forEach((d) => {
        if (d.status === "processing" && !pollRef.current[d.id]) {
          pollRef.current[d.id] = setInterval(() => pollStatus(d.id), 3000);
        }
      });
    } catch {}
    setLoading(false);
  ...
  const openChat = async (doc) => {
    ...
    try {
      const res = await chatAPI.createSession(doc.id, `Chat: ${doc.original_name}`);
      ...
      <header className="glass border-b border-ink-700/50 sticky top-0 z-10">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="font-display text-xl font-bold text-white">Research AI</span>
          </div>
          <div className="flex items-center gap-3">
            <span className="text-ink-400 text-sm hidden sm:block">
              Hello, <span className="text-white font-medium">{user?.username}</span>
            </span>
            <button onClick={logout} className="btn-ghost flex items-center gap-2 text-sm py-2 px-3">
      ...
      <main className="max-w-6xl mx-auto px-6 py-10">
        ...
        <div
          ...
          onDrop={(e) => {
            ...
            if (f) { fileRef.current.files = e.dataTransfer.files; handleUpload({ target: { files: [f] } }); }
          }}
          className={`relative card border-2 border-dashed cursor-pointer transition-all duration-300 mb-10
            ${uploading ? "border-accent-500/50 bg-accent-500/5" : "border-ink-600 hover:border-accent-500/50 hover:bg-ink-800/50"}`}
        >
          <div className="flex flex-col items-center justify-center py-10 text-center">
            {uploading ? (
              <>
                <Loader2 className="w-10 h-10 text-accent-500 animate-spin mb-4" />
                <p className="text-white font-medium mb-2">Uploading & Processing...</p>
                ...
                <p className="text-ink-400 text-sm mt-2">{uploadProgress}%</p>
              </>
            ) : (
              <>
                ...
                <p className="text-white font-medium mb-1">Drop a PDF here or click to upload</p>
                <p className="text-ink-400 text-sm">Max 50 MB · PDF only</p>
              </>
            )}
          </div>
        ...
        {error && (
          <div className="bg-red-500/10 border border-red-500/30 text-red-400 text-sm rounded-xl px-4 py-3 mb-6 animate-fade-in">
            {error}
          </div>
        )}
        ...
        <div className="flex items-center justify-between mb-5">
          <h2 className="font-display text-xl font-bold text-white">Your Library</h2>
          <button onClick={fetchDocs} className="text-ink-400 hover:text-ink-200 transition-colors">
        ...
        {loading ? (
          <div className="flex items-center justify-center py-20 text-ink-400">
            <Loader2 className="w-6 h-6 animate-spin mr-3" /> Loading documents...
          </div>
        ) : docs.length === 0 ? (
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {docs.map((doc) => {
              ...
              return (
                <div key={doc.id} className="card glass-hover flex flex-col gap-4 group animate-fade-in">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-white font-medium text-sm leading-snug truncate" title={doc.original_name}>
                        {doc.original_name}
                      </p>
                  ...
                  {doc.status === "error" && (
                    <p className="text-red-400 text-xs truncate" title={doc.error_msg}>
                      {doc.error_msg}
                    </p>
                  )}
